...
public class SHA384Test {
    ...
    @Test
    @DisplayName("SHA384 해시 알고리즘 테스트 - 문자열")
    void test_SHA384_String() throws Exception {
        String input1 = "The quick brown fox jumps over the lazy dog";
        String input2 = "The quick brown fox jumps over the lazy dog!";

        StringDigest sha384 = new SHA384();
        byte[] result1 = sha384.digest(input1);
        byte[] result2 = sha384.digest(input2, "UTF-8");
        byte[] result3 = sha384.digest(input1, StandardCharsets.UTF_8);
        ...
        for (int i = 0; i < 1000; i++) {
            byte[] result4 = sha384.digest(input1, StandardCharsets.UTF_8);
